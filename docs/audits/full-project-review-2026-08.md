...
| `src/features/sales-dashboard/server/mondaySalesCron.ts`, `scripts/sync-monday-sales-dashboard.ts` | Multiple `as never` casts suppress Supabase write/RPC typing. | Schema changes can silently break source-safe writes. | Define typed write payloads that match generated `Database` insert/update/RPC contracts; regenerate types only after remote drift is resolved. | Monday writes only quote/order fields after July 2026 and never overwrites EPCC-owned fields. |
...
- **Needs technical confirmation — remote type/schema drift:** `docs/ai-context/PROJECT_CONTEXT.md` records remote-only `sales_kpi_profit_email_sources` schema drift. The table is not in repository migrations or generated types and is not queried by application code. Do not regenerate types or create a reconciliation migration until database ownership/retention is confirmed.
...
- **Documentation update — cron schedule:** `vercel.json` is authoritative at `5 8 * * *` for EPCC and `15 8 * * *` for Monday (08:05 and 08:15 UTC). The EPCC ingestion guide says `0 10 * * *`; the July status says 10:05/10:15 SAST. Both need correction or explicit historical qualification.
...
- **Needs technical confirmation — remote schema alignment:** Static source cannot confirm whether every tracked migration is applied remotely, whether the documented remote-only `sales_kpi_profit_email_sources` object is still present, or whether remote RLS/grants match. Inspect only under a separately approved Supabase operational audit.
...
| --- | --- | --- | --- |
| `docs/epcc-profit-ingestion.md` | EPCC Vercel cron is `0 10 * * *`. | `vercel.json` schedules `/api/cron/epcc-profit` at `5 8 * * *`. | Update to 08:05 UTC or label the old schedule historical. |
| `docs/planning/WEEK_3_JULY_STATUS.md` | A unified `npm test` command exists; schedules are 10:05/10:15 SAST. | `package.json` has no `test` script; Vercel has 08:05/08:15 UTC. | Correct test/schedule claims and mark the status report historical where appropriate. |
...
- **Generated database types:** `src/types/database.types.ts` is used by repositories and Supabase clients. Its acknowledged remote drift must be resolved deliberately, not deleted or hand-maintained piecemeal.
...
- **`sales_kpi_profit_email_sources` schema-drift references:** The remote-only table is not used by current code but has a historical metadata row and unresolved ownership/retention. Do not remove documentation/type-drift notes or create deletion migration without confirmation.
...
1. Is `sales_kpi_profit_email_sources` still present remotely, who owns it, and what retention policy applies to its historical row? This determines type regeneration and any reconciliation migration.
