...
export async function AppShell({ children, pinsHubAccess: suppliedPinsHubAccess, tvMode = false, wideContent = false }: AppShellProps) {
  ...
  return (
    <div className="relative min-w-0 min-h-screen bg-background text-foreground">
      ...
      <div className={`relative z-10 flex min-h-screen flex-col ${tvMode ? "" : "md:flex-row"}`}>
        ...
        /> : null}
        <main className={`min-w-0 flex-1 ${tvMode ? "h-screen h-[100dvh] min-h-[100dvh] overflow-hidden px-4 py-4 sm:px-6" : "px-4 py-4 pt-[4.25rem] sm:px-6 md:pt-4 lg:px-8"}`}>
          <div className={`mx-auto flex min-w-0 h-full w-full flex-col gap-4 ${tvMode || wideContent ? "max-w-none" : "max-w-6xl"}`}>
            {children}
          </div>
        </main>
      </div>
