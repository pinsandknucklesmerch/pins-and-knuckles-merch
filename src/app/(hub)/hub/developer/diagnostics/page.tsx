...
export default async function DiagnosticsPage({ searchParams }: Props) {
  ...
  return <AppShell pinsHubAccess={access}><PageHeader title="Diagnostics" /><form method="get" className="flex flex-wrap gap-2"><select name="status" defaultValue={params.status ?? ""} className="h-9 rounded-md border bg-card px-2 text-sm"><option value="">Open and investigating</option><option value="open">Open</option><option value="investigating">Investigating</option><option value="resolved">Resolved</option><option value="ignored">Ignored</option></select><select name="issueType" defaultValue={params.issueType ?? ""} className="h-9 rounded-md border bg-card px-2 text-sm"><option value="">All issue types</option><option value="unassigned">Unassigned</option><option value="unmapped">Unmapped</option><option value="multi_assignee">Multi-assignee</option><option value="invalid_formula_value">Invalid formula/value</option><option value="excluded_from_member_attribution">Excluded attribution</option></select><input name="month" placeholder="YYYY-MM" defaultValue={params.month ?? ""} className="h-9 w-28 rounded-md border bg-card px-2 text-sm" /><select name="detection" defaultValue={params.detection ?? ""} className="h-9 rounded-md border bg-card px-2 text-sm"><option value="">All detection states</option><option value="current">Currently detected</option><option value="stale">No longer detected</option></select><button className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground">Filter</button></form>{source.error ? <ErrorState title="Snuggle data unavailable" message={source.error} /> : data.error ? <ErrorState title="Diagnostics unavailable" message={data.error} /> : <DiagnosticsInbox issues={data.issues} />}</AppShell>;
}
