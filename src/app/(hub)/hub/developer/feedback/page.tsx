...
export default async function FeedbackPage({ searchParams }: Props) {
  ...
  return <AppShell pinsHubAccess={access}><PageHeader title="Feedback" /><form method="get" className="flex flex-wrap gap-2"><select name="status" defaultValue={params.status ?? ""} className={nativeSelectClassName}><option value="">All statuses</option><option value="new">New</option><option value="in_progress">In progress</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select><select name="issueType" defaultValue={params.issueType ?? ""} className={nativeSelectClassName}><option value="">All types</option><option value="bug">Bug</option><option value="incorrect_data">Incorrect data</option><option value="suggestion">Suggestion</option><option value="other">Other</option></select><button className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground">Filter</button></form>{data.error ? <ErrorState title="Feedback unavailable" message={data.error} /> : <FeedbackInbox reports={data.reports} />}</AppShell>;
}
