...
export function calculateCompanyMetrics(
  ...
  const currentConversion = calculateConversionRate(current.converted, current.salesInboxEnquiries);
  const previousConversion = previous ? calculateConversionRate(previous.converted, previous.salesInboxEnquiries) : null;
  return [
