...
export function YearToDateView({ data }: { data: YearToDateData }) {
  ...
  const metrics = [["YTD Profit", gbp(data.ytdActual)], ["YTD Target", gbp(data.ytdTarget)], ["Ahead / Behind", varianceStatus], ["Target Achievement", data.achievementRate === null ? "—" : `${data.achievementRate.toFixed(1)}%`], ["Projected Year End", gbp(data.projectedYearEnd)], ["Annual Target", gbp(data.annualTarget)]];
  ...
  return <section className="grid gap-2.5" aria-labelledby="year-to-date-title">
    ...
    <div className={styles.metricGrid}>{metrics.map(([label, value]) => <Surface key={label} variant="metric" magic className={label === "Ahead / Behind" && data.variance !== null ? (data.variance >= 0 ? styles.ahead : styles.behind) : ""}><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 text-lg font-semibold tabular-nums">{value}</div></Surface>)}</div>
