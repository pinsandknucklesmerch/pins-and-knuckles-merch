...
export function YearToDateView({ data, showHeading = true, tvMode = false }: { data: YearToDateData; showHeading?: boolean; tvMode?: boolean }) {
  ...
  return <section className="grid gap-2.5" aria-labelledby={showHeading ? "year-to-date-title" : undefined} data-tv-view={tvMode ? "ytd" : undefined}>
    ...
    <div className={styles.metricGrid} data-tv-group={tvMode ? "ytd-metrics" : undefined} style={tvMode ? { "--tv-enter-index": 1 } as CSSProperties : undefined}>{metrics.map(({ label, value, format }) => <Surface key={label} variant="metric" magic className={label === "Status" && data.variance !== null ? (data.variance >= 0 ? styles.ahead : styles.behind) : ""}><div className="text-xs text-muted-foreground">{label}</div>{format ? <AnimatedMetricValue value={typeof value === "number" ? value : null} format={format} maximumFractionDigits={format === "percent" ? 1 : 0} className="mt-1 text-lg font-semibold tabular-nums" /> : <div className="mt-1 text-lg font-semibold tabular-nums">{value}</div>}</Surface>)}</div>
