...
type ExistingMonth = { quotes_done: number | null; orders_processed: number | null; sales_inbox_enquiries?: number | null; sales_inbox_decided_enquiries?: number | null; converted?: number | null } | null;
...
export function createMondayCronStore(database: SupabaseClient<Database> = serviceDatabase()): MondayCronStore {
  return {
    ...
    async readMonth(period) {
      const { data, error } = await database.from("sales_kpi_months").select("quotes_done,orders_processed,sales_inbox_enquiries,sales_inbox_decided_enquiries,converted").eq("organisation_id", MONDAY_SYNC_ORGANISATION_ID).eq("year", period.year).eq("month", period.month).maybeSingle();
...
export async function runMondaySalesCron(
  ...
  try {
    ...
    const changed = !existing || existing.quotes_done !== snapshot.quotes_done || existing.orders_processed !== snapshot.orders_processed || existing.sales_inbox_enquiries !== snapshot.sales_inbox_enquiries || existing.sales_inbox_decided_enquiries !== snapshot.sales_inbox_decided_enquiries || existing.converted !== snapshot.converted;
