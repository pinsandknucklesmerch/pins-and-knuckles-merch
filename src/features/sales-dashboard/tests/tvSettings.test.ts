...
test("TV settings map into chronological display order", () => {
  ...
  assert.deepEqual(mapTvSettingsRows(rows).map((row) => row.slideKey), ["overview", "ytd", "year_comparison", "snuggle", "team_members", "live-zoo-cam", "current-month-comparison"]);
});

test("approved fallback defaults contain all seven enabled 30-second slides", () => {
  assert.equal(DEFAULT_TV_SETTINGS.length, 7);
...
test("safe TV runtime fallback enables six slides at 30 seconds", () => {
  ...
  assert.deepEqual(settings.filter((row) => row.isEnabled).map((row) => row.slideKey), ["overview", "ytd", "year_comparison", "snuggle", "live-zoo-cam", "current-month-comparison"]);
