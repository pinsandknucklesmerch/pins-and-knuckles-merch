    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(19rem,0.85fr)]">
      ...
      {hasValidItems ? <div className="grid min-w-0 content-start gap-4 xl:row-span-2">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <Panel className="p-4"><div className="text-xs text-muted-foreground">Garment Cost</div><div className="mt-2 text-2xl font-semibold tabular-nums">{money(garmentCost)}</div></Panel>
          <Panel className="p-4"><div className="text-xs text-muted-foreground">Total Cost</div><div className="mt-2 text-2xl font-semibold tabular-nums">{money(totals)}</div><button type="button" onClick={() => void copyQuote()} className="mt-4 inline-flex items-center gap-2 text-sm text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Copy className="size-4" />Copy quote</button></Panel>
        </div>
