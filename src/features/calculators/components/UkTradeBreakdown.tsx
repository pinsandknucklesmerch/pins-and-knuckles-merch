...
export function UkTradeBreakdown({ items, results, screenSetupUnitPrice }: { items: UkTradeItemInput[]; results: UkTradeItemResult[]; screenSetupUnitPrice: number }) {
  ...
  return <details open className="group rounded-lg border border-border/90 bg-card/75 text-sm">
    ...
    <div className="grid gap-4 border-t border-border p-4">
      <dl><Row label="Print cost" value={money(breakdown.printCost)} /><Row label="Setup screens" value={String(breakdown.screenSetupCount)} /><Row label="Screen setup" value={`${breakdown.screenSetupCount} × ${money(breakdown.screenSetupUnitPrice)} = ${money(breakdown.screenSetupCost)}`} /><Row label="Embroidery cost" value={money(breakdown.embroideryCost)} /><Row label="Embroidery setup" value={money(breakdown.embroiderySetupCost)} /><Row label="Total unit cost excl. VAT" value={`${money(breakdown.unitCost)} / unit`} /><Row label={`Subtotal (${breakdown.quantity} units, excl. VAT)`} value={money(breakdown.total)} strong /></dl>
      <div className="border-t border-border pt-4"><h2 className="mb-3 text-sm font-semibold text-foreground">Item Breakdown</h2><div className="grid gap-3">{breakdown.validItems.map(({ input, result }) => <section key={result.itemId} className="rounded-md border border-border/70 bg-background/55 p-3"><div className="mb-2 flex items-baseline justify-between gap-3"><h3 className="min-w-0 truncate text-sm font-semibold text-foreground">{getEuItemLabel(input.itemLabel, items.indexOf(input))}</h3><span className="shrink-0 text-xs text-muted-foreground">× {result.quantity}</span></div><dl><Row label="Garment base price / unit" value={`${money(result.garmentCost / result.quantity)} / unit`} />{result.printBreakdowns.map((print) => <Row key={`${print.position}-${print.colourCount}`} label={printLabel(print.position, print.colourCount)} value={`${money(print.unitPrice)} / unit`} />)}<Row label="Setup screens" value={String(result.screenSetupCount)} /><Row label="Screen setup" value={`${result.screenSetupCount} × ${money(breakdown.screenSetupUnitPrice)} = ${money(result.screenSetupCost)}`} />{result.embroideryBreakdowns.map((embroidery) => <Row key={embroidery.stitches} label={`Embroidery ${embroidery.stitches.toLocaleString()} stitches`} value={`${money(embroidery.unitPrice)} / unit`} />)}<Row label="Embroidery setup" value={money(result.embroiderySetupCost)} /><Row label="Total unit cost excl. VAT" value={`${money(result.totalCost / result.quantity)} / unit`} /><Row label="Item total" value={money(result.totalCost)} strong /></dl></section>)}</div></div>
    </div>
  </details>;
}
