...
export function calculateUkTradeItem(input: UkTradeItemInput, data: UkTradeReferenceData): UkTradeItemResult {
  ...
  for (const selection of input.printPositions) {
    ...
    printCost += tier.unitPrice * input.quantity;
    screens += tier.setupScreenCountStrategy === "colour_count" ? (colourCount ?? 0) + 1 : tier.setupScreenCountStrategy === "one" ? 2 : 0;
  }
  ...
  for (const stitches of input.embroideryStitches.filter((value): value is number => value !== null)) {
    ...
    embroideryCost += (basePrice + extra * (extraPrice ?? 0)) * input.quantity; embroiderySetupCost += embroiderySetup ?? 0;
  }
  ...
  return { itemId: input.id, garmentId: garment?.id ?? "", quantity: input.quantity, garmentCost, printCost, screenSetupCount: screens, screenSetupCost, embroideryCost, embroiderySetupCost, totalCost, errors };
}
