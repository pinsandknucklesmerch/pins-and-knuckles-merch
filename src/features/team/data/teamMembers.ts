...
type ProfileRecord = { full_name: string | null; email: string | null; created_at: string | null; last_active_at: string | null };
...
export function resolveLastActive(lastActiveAt: string | null, lastSignInAt: string | null) {
  return lastActiveAt ?? lastSignInAt ?? null;
}
...
export async function getTeamMembers(organisationId: string): Promise<TeamMember[]> {
  ...
  const { data: rawMemberships, error } = await membershipQuery
    .select("id,role,user_id,is_active,monday_member_id,created_at,profiles!organisation_members_user_id_fkey(full_name,email,created_at,last_active_at),app_access(app_key,access_level)")
  ...
  return memberships.map((member, index) => {
    ...
    return {
      ...
      joinedDate,
      lastActive: resolveLastActive(profile?.last_active_at ?? null, authUser?.last_sign_in_at ?? null),
