...
export function GarmentsManager({ garments, productTypes, accessLevel }: { garments: GarmentRecord[]; productTypes: ProductTypeRecord[]; accessLevel: AccessLevel }) {
  ...
  return <div className="space-y-4">
    ...
    <Surface className="overflow-x-auto bg-card/70 p-0"><table className="min-w-[1050px] text-left text-sm"><thead className="bg-secondary/60 text-xs text-muted-foreground"><tr><th className="px-4 py-3">Edit</th><Header label="Code" sortKey="code" sort={sort} direction={direction} onClick={changeSort} /><Header label="Alt Code" sortKey="altCode" sort={sort} direction={direction} onClick={changeSort} /><Header label="Brand" sortKey="brand" sort={sort} direction={direction} onClick={changeSort} /><Header label="Name" sortKey="name" sort={sort} direction={direction} onClick={changeSort} /><th className="px-4 py-3">Colour</th><th className="px-4 py-3">Tags</th><Header label="EUR Price" sortKey="eurBasePrice" sort={sort} direction={direction} onClick={changeSort} /><Header label="GBP Price" sortKey="gbpPrice" sort={sort} direction={direction} onClick={changeSort} /><Header label="Extra Size Cost" sortKey="extraSizeCost" sort={sort} direction={direction} onClick={changeSort} /><Header label="Product Type" sortKey="productTypeName" sort={sort} direction={direction} onClick={changeSort} /></tr></thead><tbody>{visible.length ? visible.map((item) => <tr key={item.id} className="border-t border-border/70"><td className="px-4 py-3">{canWrite ? <button type="button" onClick={() => setSelected(item)} className="text-primary hover:underline">Edit</button> : "—"}</td><td className="px-4 py-3 font-mono text-xs">{item.code}</td><td className="px-4 py-3">{display(item.altCode)}</td><td className="px-4 py-3">{display(item.brand)}</td><td className="px-4 py-3 font-medium">{item.name}</td><td className="px-4 py-3">{display(item.colour)}</td><td className="max-w-48 truncate px-4 py-3">{display(item.tags)}</td><td className="px-4 py-3 text-right tabular-nums">{formatGarmentCurrency(item.eurBasePrice, "EUR")}</td><td className="px-4 py-3 text-right tabular-nums">{formatGarmentCurrency(item.gbpPrice, "GBP")}</td><td className="px-4 py-3 text-right tabular-nums">{display(item.extraSizeCost)}</td><td className="px-4 py-3">{display(item.productTypeName)}</td></tr>) : <tr><td colSpan={11} className="px-4 py-10 text-center text-muted-foreground">No garments.</td></tr>}</tbody></table></Surface>
  </div>;
}
