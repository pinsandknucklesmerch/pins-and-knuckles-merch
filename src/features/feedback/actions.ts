...
export async function submitFeedbackReport(input: { issueType: string; comment: string; attemptedAction?: string; pageRoute: string; userAgent?: string }): Promise<FeedbackActionState> {
  ...
  const { error } = await supabase.rpc("submit_hub_feedback_report", { p_issue_type: input.issueType as FeedbackIssueType, p_comment: comment, p_attempted_action: attemptedAction, p_page_route: route, p_user_agent: input.userAgent?.slice(0, 1000) || null });
...
export async function updateFeedbackReport(input: { id: string; status: string; developerNotes: string }): Promise<FeedbackActionState> {
  ...
  const supabase = await createClient();
  const { error } = await supabase.rpc("update_hub_feedback_report", { p_id: input.id, p_status: input.status as FeedbackStatus, p_developer_notes: input.developerNotes.trim() || null });
