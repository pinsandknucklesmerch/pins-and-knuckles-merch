...
export function summarizeMonthlySalesBoard(board: Pick<MondayBoard, "id" | "name">, items: MondayItem[]) {
  ...
  for (const item of items) {
    ...
    const date = parseMondayDate(column(item, MONTHLY_SALES_COLUMNS.dateInTouch)?.text ?? column(item, MONTHLY_SALES_COLUMNS.dateInTouch)?.value); const dateMatchesBoardMonth = Boolean(date && expectedMonth && date.startsWith(`${expectedMonth.year}-${String(expectedMonth.month).padStart(2, "0")}-`));
    included.push({ item, people: people(column(item, MONTHLY_SALES_COLUMNS.people)), channel: column(item, MONTHLY_SALES_COLUMNS.channel)?.text?.trim() || null, channelNormalized: normalizeStatus(column(item, MONTHLY_SALES_COLUMNS.channel)?.text), converted: normalizeStatus(column(item, MONTHLY_SALES_COLUMNS.converted)?.text) === "yes", date, dateMatchesBoardMonth });
  }
  ...
  const missingDates = included.filter((item) => !item.date).map((item) => ({ id: String(item.item.id), name: item.item.name, group: item.item.group?.title ?? null })); const mismatchedDates = included.filter((item) => item.date && !item.dateMatchesBoardMonth).map((item) => ({ id: String(item.item.id), name: item.item.name, group: item.item.group?.title ?? null, dateInTouch: item.date }));
